...
def part_2(inp):
    ...
    lazy_sum_of_surface = sum([(6 - sum([abs(p1[0]-p2[0])+abs(p1[1]-p2[1]) +
                                         abs(p1[2]-p2[2]) == 1 for p2 in points])) for p1 in points])
    ...
    points_trapped = list(map(tuple, np.argwhere(world3 == True)))

    points = set([tuple(map(int, line.split(','))) for line in inp])
    lazy_sum_of_surface_points_trapped = sum([(6 - sum([abs(p1[0]-p2[0])+abs(p1[1]-p2[1]) +
                                                        abs(p1[2]-p2[2]) == 1 for p2 in points_trapped])) for p1 in points_trapped])
