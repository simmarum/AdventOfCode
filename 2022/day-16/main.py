    ...
    for t in range(1, max_step+1):
        ...
        for node, opened, pressure in states:
            ...
            if (node not in opened) and (rate > 0):
                new_states.append(
                    (node, tuple(sorted(set(opened + (node,)))), pressure+(rate*(max_step-t))))
...
def part_2(inp):
    ...
    for t in range(1, max_step+1):
        ...
        for node1, node2, opened, pressure in states:
            ...
            if (nodes_id[node1] & opened == 0) and (rate1 > 0) and (nodes_id[node2] & opened == 0) and (rate2 > 0) and (node1 != node2):
                new_states.add(
                    (node1, node2, opened | nodes_id[node1] | nodes_id[node2], pressure+(rate1*(max_step-t))+(rate2*(max_step-t))))
            else:
                tmp_opened = opened
                if (nodes_id[node1] & tmp_opened == 0) and (rate1 > 0):
                    ...
                    for c2 in children2:
                        new_states.add(
                            (*sorted([node1, c2]), tmp_opened, pressure+(rate1*(max_step-t))))

                if (nodes_id[node2] & tmp_opened == 0) and (rate2 > 0):
                    tmp_opened = tmp_opened | nodes_id[node2]
                    for c1 in children1:
                        new_states.add(
                            (*sorted([c1, node2]), tmp_opened, pressure+(rate2*(max_step-t))))

            for child1, child2 in product(children1, children2):
                new_states.add(
                    (*sorted([child1, child2]), opened, pressure))
        states = new_states
